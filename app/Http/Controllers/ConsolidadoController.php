...
class ConsolidadoController extends Controller
{
    public function consolidadoCensados(Request $request){
        ...
        if($corregimiento == "todo"){
            ...
            if($corregimiento != "null"){
                ...
                ->join("vivienda_hogar", "informacion_personal.identificacion", "vivienda_hogar.identificacion_jefe")
                ->where("rol", "Jefe de hogar")
                ->whereNull("informacion_personal.corregimiento")
                ->where("informacion_personal.estado", 1)
                ->offset($offset)
                ->limit($limit) 
                ->orderBy("informacion_personal.numero_caracterizacion")
    ...
    public function municipiosConsolidado(){
        ...
        $municipios_no_registra = DB::connection('mysql')->table('informacion_personal')
        ->join("educacion", "informacion_personal.identificacion", "educacion.identificacion_individuo")
        ->join("situacion_laboral", "informacion_personal.identificacion", "situacion_laboral.identificacion_individuo")
        ->where("informacion_personal.estado", 1)
        ->whereNull('municipio')
        ->orWhereNull('departamento')
        ->orWhereNull('corregimiento')
        ->orderBy("informacion_personal.numero_caracterizacion")
        ->count();

        $municipios[] = [
            "cantidad" => $municipios_no_registra,
    ...
    public function exportarConsolidadoExcel(Request $request) {
        ...
        if($idCorregimiento == "todo"){
            ...
            $corregimiento_en = DB::connection('mysql')->table('corregimientos')
            ->where("id", $idCorregimiento)
            ->first();

            if($corregimiento_en){
                $corregimiento = $corregimiento_en->nombre;
            }else{
                $corregimiento = "No registra";
            }
        }
    ...
    public function consultarIntegrantesJefes($idCorregimiento){
        ...
        foreach ($integrantes as $integrante) {
           ...
           if($bandera == 0){
                $individuos_sin_jefe[] = $integrante;
           }
        }

        $numero_familia = 1;
        foreach ($individuos_sin_jefe as $individuo) {
            ...
            $individuo->numero_familia = $numero_familia;
            $individuo->rowspan = $rowspan;
            $individuo->numero_orden = $numero_orden;
